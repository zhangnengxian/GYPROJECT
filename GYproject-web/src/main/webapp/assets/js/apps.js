...
generateSlimScroll = function(element) {
    ...
    if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
    	...
    	if('function' === typeof $.fn.slimScroll){
            $(element).slimScroll(scrollBarOption);
        }else{
            $.getScript("assets/plugins/slimscroll/jquery.slimscroll.min.js").done(function(){
                $(element).slimScroll(scrollBarOption);
            });
        }
    }
...
handlePanelAction = function() {
    ...
    $(document).on('click', '[data-click=panel-expand]', function(e) {
        e.preventDefault();
        var target = $(this).closest('.panel'),
        targetBody = $(target).find('.panel-body'),
        targetTop = 40;
        if ($(targetBody).length !== 0) {
            var targetOffsetTop = $(target).offset().top,
            targetBodyOffsetTop = $(targetBody).offset().top;
            targetTop = targetBodyOffsetTop - targetOffsetTop;
        }
        
        if ($('body').hasClass('panel-expand') && $(target).hasClass('panel-expand')) {
            $('body, #ajax-content, .panel').removeClass('panel-expand');
            $('.panel').removeAttr('style');
            $(targetBody).removeAttr('style');
        } else {
            $('body, #ajax-content').addClass('panel-expand');
...
handleDraggablePanel = function() {
    ...
    targetHandle = '.panel-heading.draggable',
...
handleThemePageStructureControl = function() {
    ...
    $('.theme-panel [name=header-styling]').on('change', function() {
        ...
        targetClassRemove = ($(this).val() == 1) ? 'navbar-inverse' : 'navbar-default';
...
/* 13. Handle Save Panel Position Function - added in V1.5
------------------------------------------------ */
handleSavePanelPosition = function(element) {
    "use strict";
...
handleLocalStorage = function() {
    ...
    if (typeof(Storage) !== 'undefined' && typeof(localStorage) !== 'undefined') {
        ...
        alert('该浏览器不支持本地存储，推荐您使用Chrome、Firefox、IE9+等其他现代浏览器！或者如果页面右键菜单有极速模式选项，也可将当前浏览器切换为极速模式。'); 
    }
},
...
/* 16. Handle Ajax Page Load - added in V1.5
------------------------------------------------ */
default_content = '<div class="p-t-40 p-b-40 text-center f-s-20 content"><i class="fa fa-warning fa-lg text-muted m-r-5"></i> <span class="f-w-600 text-inverse">Error 404! Page not found.</span></div>',
...
handleLoadPage = function(hash) {
    ...
    if(targetUrl.indexOf('&') > -1){
        ...
        if(targetUrl.indexOf('=') > -1){
            ...
            loadThisPage('',targetUrl);
        }
    }
},
pageLoadRequest = '',
loadThisPage = function(target,targetUrl){
	pageLoadRequest && pageLoadRequest.abort();
    var t = '';
    !target || $('#' + target).length < 1 ? t = $('#ajax-content') : t = $('#' + target);
    //重置选中行数据全局变量
	trSData = {};
	//移除首屏检索工具
	$(".project-filter").remove();
	//卸载关联操作
	$(".attach-panel").removeAttachOper();
    t.fadeOut(200,function(){
    	pageLoadRequest && pageLoadRequest.abort();
    	pageLoadRequest = $.ajax({
            type: 'POST',
            ...
            success: function(data) {
                ...
                if(t.find(".no-equal-height").length < 1) t.panelEqualHeight();
                var url = location.href,
                    QRSize = 110;
                if(url.indexOf("localhost") > -1 || url.indexOf("127.0.0.1") > -1){
            		$(".qrcode-desc").hide();
            	}else{
            		$(".qrcode-desc").css({"font-size": QRSize * 0.125 + "px"}).show();
            		$("#qrcodebox").html(generateQR({
                    	text: url,
                        width: QRSize,
                        height: QRSize
                    }));
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                t.html(default_content).fadeIn(200);
            }
        });
    });
},
...
handleCheckPageLoadUrl = function(hash) {
	if($("body").is(".adminPanel") && location.hash === '') return;
//    hash = (hash) ? hash : '#ajax/index_v2.html';
//    hash = (hash) ? hash : $("body").is(".adminPanel") ? '#system/dashboard' : '#projectSummary/main';
    hash = (hash) ? hash : $("body").is(".adminPanel") ? '#dept/main' : '#welcome/main';
    if (hash === '') {
...
/* 18. Handle Ajax Sidebar Toggle Content - added in V1.5
------------------------------------------------ */
handleSidebarAjaxClick = function() {
    $(document).on("click", '.sidebar [data-toggle=ajax]', function() {
        var targetLi = $(this).closest('li'),
        targetParentLi = $(this).parents();
        $('.sidebar li').not(targetLi).not(targetParentLi).not(".sidebar-mainmenu li").removeClass('active');
...
handleUnlimitedTabsRender = function() {
    ...
    function handleTabOverflowScrollWidth(obj, animationSpeed) {
        ...
        viewWidth = $(obj).width(),
        prevWidth = $(obj).find('li.active').width(),
        speed = (animationSpeed > -1) ? animationSpeed : 150,
        fullWidth = 0;
    ...
    function handleTabButtonAction(element, direction) {
        ...
        marginLeft = parseInt($(obj).find('.nav.nav-tabs').css('margin-left')),
    ...
    $(document).on('click', '[data-click="next-tab"]', function(e) {
        e.preventDefault();
        handleTabButtonAction(this,'next');
    });
    
    // handle tab prev button click action
    $(document).on('click', '[data-click="prev-tab"]', function(e) {
        e.preventDefault();
...
handleMobileSidebar = function() {
    ...
    $('.page-sidebar-minified .sidebar [data-scrollbar=true] a').on('touchmove',function(e){
        ...
        if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
            var touch = e.originalEvent.touches[0] || e.originalEvent.changedTouches[0],
            touchVertical = touch.pageY,
            elementTop = touchVertical - oriTouch;
    ...
    $('.page-sidebar-minified .sidebar [data-scrollbar=true] a').on('touchend', function(e) {
        ...
        sidebarTopPosition = parseInt($('#sidebar').css('padding-top')),
        sidebarContainerHeight = $('#sidebar').height();
...
handleUnlimitedTopMenuRender = function() {
    ...
    function handleMenuButtonAction(element, direction) {
        ...
        containerWidth = $('.top-menu').width() - 88,
        totalWidth = 0,
        finalScrollWidth = 0;
    ...
    function handlePageLoadMenuFocus() {
        ...
        targetList = $('.top-menu .nav > li'),
        targetActiveList = $('.top-menu .nav > li.active'),
        targetContainer = $('.top-menu'),
        
        marginLeft = parseInt($(targetMenu).css('margin-left')),
        viewWidth = $(targetContainer).width() - 128,
        prevWidth = $('.top-menu .nav > li.active').width(),
        speed = 0,
        fullWidth = 0;
...
handleTopMenuSubMenu = function() {
    ...
    $('.top-menu .sub-menu .has-sub > a').click(function() {
        ...
        otherMenu = $(this).closest('ul').find('.sub-menu').not(target);
        $(otherMenu).not(target).slideUp(250, function() {
            $(this).closest('li').removeClass('expand');
        });
        $(target).slideToggle(250, function() {
