...
DataTable.Buttons.swfPath = 'assets/plugins/DataTables/extensions/Buttons/swf/flashExport.swf';
...
DataTable.ext.buttons.excelFlash = $.extend( {}, flashButton, {
	...
	action: function ( e, dt, button, config ) {
		...
		var addTitle = function ( row ) {
			console.info(row);
			return '<row><c t="inlineStr"><is><t>' + row.join('') + '</t></is></c></row><row></row>';
		};
		
		var addRemark = function ( row ) {
			console.info(row);
			var rows = [];
			
			for ( var i=0, ien=row.length ; i<ien ; i++ ) {
				rows.push( '<row><c t="inlineStr"><is><t>'+row[i]+'</t></is></c></row>' );
			}

			return rows.join('') + '<row></row>';
		};
		...
		if ( config.exportOptions.title ) {
			xml += addTitle( config.exportOptions.title );
		}

		console.info(1.0);

		if ( config.exportOptions.remark ) {
			xml += addRemark( config.exportOptions.remark );
		}
