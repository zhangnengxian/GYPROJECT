    ...
    if ($('#govAuditCostTable').length !== 0) {
    	govAuditCostTable= $('#govAuditCostTable').on( 'init.dt',function(){
   			...
   			$("#govAuditCostTable_filter input").on("change",function(){
   				searchData={};
   				searchData.projNo=$("#govAuditCostTable_filter input").val();searchData.menuId = menuId;
   			...
   			registerMonitor();
   			setTimeout(function(){
   			    $("#govAuditCostTable").DataTable().columns.adjust();
			...
			serverSide:true,
            ajax: {
                url: 'govAuditCost/queryProjectSgac',
                type:'post',
                data: function(d){
...
var searchPop = function(){
	$('.searchBtn').on("click",function(){
		var url = "#govAuditCost/settleSearchPopPage";
		var popoptions = {
			title: '查询',
			content: url,
			accept: searchDone
...
var searchDone= function(){
	 searchData= $("#searchForm_reportAudit").serializeJson();searchData.menuId = menuId;
