    ...
    if ($('#recordListTable').length !== 0) {
    	recordListTable= $('#recordListTable').on( 'init.dt',function(){
            ...
            columns: [
				...
				{"data":"backfillSoil"},
				{"data":"tampingMethod"},
				{"data":"orPavementStruc"},
				{"data":"orPavementThick",className:"text-right"},
				{"data":"rePavementStruc"},
				{"data":"rePavementThick",className:"text-right"}
			],
			columnDefs: [{
				"targets":0,
...
$(".deleteBtn2").on("click",function(){
	...
	var dataRows=info.recordsTotal;
	 ...
	 if(len>0){
		 var rows =trSData.recordListTable.json;
		 if(rows.tbId){
			 delRowData = delRowData + rows.tbId+',';
		 }
