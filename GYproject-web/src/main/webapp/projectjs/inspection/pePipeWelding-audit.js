...
var handleRecord = function() {
    ...
    if ($('#recordListTable').length !== 0) {
    	recordListTable= $('#recordListTable').on( 'init.dt',function(){
           ...
           // ajax: 'projectjs/inspection/json/pePipeWelding.json',
            responsive: {
            	details: {
            ...
            columns: [
	  			...
	  			{"data":"peWeldingDate"}, 
				{"data":"pePipePosition"},
				{"data":"peSectionLen",className:"text-right"},
				{"data":"jointNum",className:"text-right"},
				{"data":"peHeatingTime",className:"text-right"},
				{"data":"peHeatingOc",className:"text-right"},
				{"data":"holdingTime",className:"text-right"},
				{"data":"peCoolingTime",className:"text-right"},
				{"data":"otherInpect"}
			],
			columnDefs: [{
				"targets":0,
...
var pePipeWeldingAndAudit = function () {
	"use strict";
    return {
        //main function
    	init: function () {
...
$(".deleteBtn2").on("click",function(){
	...
	var dataRows=info.recordsTotal;
	 ...
	 if(len>0){
		 ...
		 if(rows.peId){
			 delRowData = delRowData + rows.peId+',';
			 json1.peId = rows.peId;
...
var sureClose=function(){
	...
	$.ajax({
		type:'post',
		url:$("#actionName").val()+'/saveSignNoticeReocrd',
		contentType: "application/json;charset=UTF-8",
        data: cwId,
        success:function(data){
        	console.info(data);
