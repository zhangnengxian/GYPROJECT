    ...
    if ($('#recordListTable').length !== 0) {
    	recordListTable= $('#recordListTable').on( 'init.dt',function(){
            ...
            columns: [
				...
				{"data":"pileNo"},
				{"data":"hwSpec"},
				{"data":"weldNum",className:"text-right"},
				{"data":"material"},
				{"data":"obstacle",className:"text-right"},
				{"data":"dealSituation"},
				{"data":"hwDate"}
			],
			columnDefs: [{
			  	...
			  	success: function (data) {
                    ...
                    if(data === "false"){
                        ...
                        content = "此信息已被修改，请刷新页面！";
			  			...
			  			addDataRow.hwId = data;
			  			$('#hwId1').val(data);
			  			
			  			if(_inApk && $(".attach-images-list").find(".new-image").length){
			  				preImagesUpload($(".attach-images-list .uploadBtn"),  $('#projId').val(), $('#projNo').val(),  getStepId(), '');
			  			};
                        showReport();
                        successBack();
                    }
...
$(".deleteBtn2").on("click",function(){
	...
	var dataRows=info.recordsTotal;
	 ...
	 if(len>0){
		 var rows =trSData.recordListTable.json;
		 if(rows.hwId){
			 delRowData = delRowData + rows.hwId+',';
		 }
