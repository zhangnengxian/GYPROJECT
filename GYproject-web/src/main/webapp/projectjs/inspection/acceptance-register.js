...
var addBtnCheck = function(){
	var post=$("#loginPost").val();  //当前登录人职务-施工员、焊工、班组、验收员
	if(post.indexOf(',61,')>=0 || post.indexOf(',63,')>=0 || post.indexOf(',64,')>=0 || post.indexOf(',11,')>=0|| post.indexOf(',126,')>=0){
...
var queryBack=function(data){
	...
	if(data.flag==1){
		...
		if(post.indexOf(builder)>=0 || post.indexOf(',11,')>=0){
	...
	if(post.indexOf(',11,')>=0){   //如果是管理员，显示删除按钮并隐藏增加按钮，否则隐藏删除按钮
		$(".deleteBtn").removeClass("hidden");
//		$(".addBtn").removeClass("hidden");
