    	...
    	recordListTable= $('#recordListTable').on( 'init.dt',function(){
           ...
           // ajax:'projectjs/inspection/json/electrode_baking_list.json',
            ...
            columns: [
	  			...
	  			{"data":"pipeSectionNo"}, 
	  			{"data":"pipingSpec"}, 
				{"data":"pipeLen",className:"text-right"},
				{"data":"installPosition"},
				{"data":"pipeTopLevel",className:"text-right"},/*
				{"data":"pipeSlope",className:"text-right"},*/
				{"data":"buriedDepth",className:"text-right"},
				{"data":"equipmentName"},
				{"data":"equipmentSpec"},
				{"data":"equipmentPileNo"},
				{"data":"equipmentBuriedDepth",className:"text-right"},
				{"data":"installDate"}
			],
			columnDefs: [{
				"targets":0,
    	...
    	init: function () {
         	...
         	$("#listTab, #auditTab, #signTab").off("shown.bs.tab").on("shown.bs.tab",function(){
         		...
         		if($(this).is($("#listTab,#signTab"))){
         			...
         			//记录列表
         			if(!$.fn.DataTable.isDataTable('#auditTab')){
         				handleRecord();
         			}else{
         				$('#auditTab').cgetData(true);
         			}*/
         			//记录列表
...
$(".deleteBtn2").on("click",function(){
	...
	var dataRows=info.recordsTotal;
	 ...
	 if(len>0){
		 var rows =trSData.recordListTable.json;
		 if(rows.eiId){
			 delRowData = delRowData + rows.eiId+',';
		 }
