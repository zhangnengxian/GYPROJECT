...
var handleRecord = function(){
    ...
    if ($('#recordListTable').length !== 0) {
    	recordListTable= $('#recordListTable').on( 'init.dt',function(){
           ...
           // ajax:'projectjs/inspection/json/pipeline_install_list.json',
            responsive: {
            	details: {
            ...
            columns: [
				...
				{"data":"weldMethod"},
				{"data":"weldMaterialNo"},
				{"data":"weldMaterialSize",className:"text-right"},
				{"data":"weldLayer",className:"text-right"},
				{"data":"powerStage",className:"text-right"},
				{"data":"weldCurrent",className:"text-right"},
				{"data":"arvoltage",className:"text-right"},
				{"data":"weldSpeed",className:"text-right"},
				{"data":"weldTime",className:"text-right"},
				{"data":"isInspect"},
			],
			columnDefs: [{
				"targets":0,
...
var showRecordPhoto = function(){
	 ...
	 if(len>0){
		 ...
		 $('#pliId1').val(rows.pliId);
		 $('#pcId1').val(rows.pliId);
		 recordId = rows.pliId;
		 pcId = rows.pcId;
	 }
	if(recordId){
...
$(".deleteBtn2").on("click",function(){
	...
	var dataRows=info.recordsTotal;
	 ...
	 if(len>0){
		 var rows =trSData.recordListTable.json;
		 if(rows.pliId){
			 delRowData = delRowData + rows.pliId+',';
		 }
