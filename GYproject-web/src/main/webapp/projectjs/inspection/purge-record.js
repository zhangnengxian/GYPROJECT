    	...
    	recordListTable= $('#recordListTable').on( 'init.dt',function(){
            ...
            columns: [
                     {"data":"purgeId"},
      	  			...
      	  			{"data":"pileNo"}, 
      				{"data":"purgeSpec"},
      				{"data":"purgeLen",className:"text-right"},
...
var showRecordPhoto = function(){
	 ...
	 if(len>0){
		 ...
		 $('#purgeId1').val(rows.purgeId);
		 $('#pcId1').val(rows.pcId);
		 recordId = rows.purgeId;
		 pcId = rows.pcId;
	 }
	if(recordId){
...
$(".deleteBtn2").on("click",function(){
	...
	var dataRows=info.recordsTotal;
	 ...
	 if(len>0){
		 ...
		 if(rows.purgeId){
			 delRowData = delRowData + rows.purgeId+',';
			 json1.purgeId = rows.purgeId;
