    	...
    	recordListTable= $('#recordListTable').on( 'init.dt',function(){
           ...
           // ajax:'projectjs/inspection/json/electrode_baking_list.json',
            ...
            columns: [
	  			{"data":"aiId"}, 
	  			{"data":"buriedPileNo"}, 
	  			{"data":"soilResistance",className:"text-right"}, 
				{"data":"buriedDepth",className:"text-right"},
				{"data":"wallSpacing",className:"text-right"},
				{"data":"anodeSpacing",className:"text-right"},
				{"data":"workPotential",className:"text-left"},
				{"data":"installDate",className:"text-left"}
			],
			columnDefs: [{
				"targets":0,
...
$(".deleteBtn2").on("click",function(){
	...
	var dataRows=info.recordsTotal;
	 ...
	 if(len>0){
		 ...
		 if(rows.aiId){
			 delRowData = delRowData + rows.aiId+',';
			 json1.aiId = rows.aiId;
