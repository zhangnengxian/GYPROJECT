...
var handleRecord = function(){
    ...
    if ($('#recordListTable').length !== 0) {
    	recordListTable= $('#recordListTable').on( 'init.dt',function(){
           ...
           //ajax:'projectjs/inspection/json/electrode_record_list.json',
            responsive: {
            	details: {
...
$(".deleteBtn2").on("click",function(){
	...
	var dataRows=info.recordsTotal;
	 ...
	 if(len>0){
		 ...
		 if(rows.erId){
			 delRowData = delRowData + rows.crId+',';
			 json1.erId = rows.erId;
