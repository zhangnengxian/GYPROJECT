    ...
    if ($('#recordListTable').length !== 0) {
    	recordListTable= $('#recordListTable').on( 'init.dt',function(){
            ...
            columns: [
	  			...
	  			{"data":"pipeNo"}, 
				{"data":"pileNo"},
				{"data":"topElevation",className:"text-right"},
				{"data":"soilThick",className:"text-right"},
				{"data":"isDamage"},
				{"data":"damageNum",className:"text-right"},
				{"data":"damageArea",className:"text-right"},
				{"data":"repairNum",className:"text-right"},
				{"data":"repairArea",className:"text-right"}
			],
			columnDefs: [{
				"targets":0,
 ...
 * 页面初始化
 */
var undergroundInpect = function(){
	"use strict";
    return {
        //main function
    	init: function () {
...
$(".deleteBtn2").on("click",function(){
	...
	var dataRows=info.recordsTotal;
	 ...
	 if(len>0){
		 var rows =trSData.recordListTable.json;
		 if(rows.ugiId){
			 delRowData = delRowData + rows.ugiId+',';
		 }
