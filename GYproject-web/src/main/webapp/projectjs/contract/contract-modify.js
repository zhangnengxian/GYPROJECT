...
var scaleTable;
var menuId = "110402";//修改合同右侧详细页面使用合同签订的右侧详细页面，使用此菜单ID传递到后台拼接成key查询出对应的右侧页面
/**
 * 加载工程列表
 */
var handleConstructContract = function(){
    ...
    if ($('#constructContractTable').length !== 0) {
    	constractTable= $('#constructContractTable').on( 'init.dt',function(){
    		//加载右侧页面
   			$('#construct_contract_panel_box').cgetContent('contractModify/viewPage',{projId:'',projectType:'',corpId:'',menuId:menuId});
   			//默认选中第一行
    		$(this).bindDTSelected(trSelectedBack,true);
             ...
             buttons: [
                 ...
                 { text: '修改', className: 'btn-sm btn-query signBtn' }
             ],
             //启用服务端模式，后台进行分段查询、排序
			 serverSide:true,
             ajax: {
                url: 'contractModify/queryContract',
                type:'post',
                data: function(d){
...
var searchMonitor = function(){
	$('.searchBtn').off('click').on('click',function(){
		defaultValue();
		var url = '#contractModify/contractModifySearchPopPage';
		//加载弹屏
		$('body').cgetPopup({
			title: '查询',
			content: url,
...
var trSelectedBack = function(v, i, index, t, json){
    ...
    //加载右侧页面
    $("#construct_contract_panel_box").cgetContent("contractModify/viewPage",{projId:json.projId,projectType:json.projectType,corpId:json.corpId,menuId:menuId},function(){
...
var signMonitor=function(){
	$('.signBtn').off('click').on('click',function(){
		...
		if((trSData.constructContractTable.json.isPay) =='1'){   //若有收款，则付款比例不可以更改
			$("#payType").attr("disabled",true);
			$("#payType").attr("readonly",true);
			$("input[type='radio'][name='isSpecial']").attr("disabled",true);
		}else{
			$("#payType").attr("disabled",false);
			$("#payType").attr("readonly",false);
		}
