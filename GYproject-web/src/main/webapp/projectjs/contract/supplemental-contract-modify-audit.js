    ...
    if ($('#supplementalContractAuditTable').length !== 0) {
    	$('#supplementalContractAuditTable').on( 'init.dt',function(){
   		...
   		//跳转链接
			...
			serverSide:true,
            ajax: {
                url: 'supplementContractModifyAudit/querySupplementalModify',
                type:'post',
                data: function(d){
...
var searchMonitor = function(){
	$(".searchBtn").off("click").on("click",function(){
		var url = "#supplementContractModifyAudit/searchPopPage";
		var popoptions = {
			title: '查询',
			content: url,
			accept: searchDone
...
var auditBtnMonitor = function(){
	$(document).off("click", ".level").on("click", ".level", function(){
		...
		var data = {"scId":scId,"currentLevel":currentLevel,"isAudit":isAudit,"corpId":corpId};
		console.log("加载审核屏");
		$("#ajax-content").cgetContent("supplementContractModifyAudit/auditPage",data);
	});
