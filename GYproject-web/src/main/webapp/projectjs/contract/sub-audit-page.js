...
var handleAuditHistory = function() {
    ...
    if ($('#auditHistoryTable').length !== 0) {
    	$('#auditHistoryTable').on( 'init.dt',function(){
			...
			serverSide:true,
            ajax: {
                url: 'subContract/queryManageRecord',
                type:'post',
                data: function(d){
...
var contractDetailPage = function(){
	var scId = $(".scId").val();
	var data = {"scId":scId};
	$("#contractAudit_panelBox").cgetContent("subContract/auditViewPage",data,handleAuditHistory);
	$("#sub_safe_audit").cgetContent("subContract/safeAuditViewPage",data);
}
