...
var histSearchData = {"projId":$("#projId").val(),"businessOrderId":$("#businessOrderId").val()};
var handleAuditHistory = function() {
	"use strict";
    if ($('#auditHistoryTable').length !== 0) {
    	histSearchData = {"projId":projId,"businessOrderId":businessOrderId};
    	$('#auditHistoryTable').on( 'init.dt',function(){
			...
			serverSide:true,
            ajax: {
                url: 'imcModifyAudit/queryManageRecord',
                type:'post',
                data: function(d){
...
var contractDetailPage = function(){
	var imcId = $(".imcId").val();
	var data = {"imcId":imcId};
	$("#contractAudit_panelBox").cgetContent("imcModifyAudit/viewPage",data,handleAuditHistory);
}
