    ...
    if ($('#planEstablishTable').length !== 0) {
    	$('#planEstablishTable').on( 'init.dt',function(){
    		...
    		$(this).bindDTSelected(trSelectedBack,true);
   			//加载页面
   			$("#plan_establish_panel_box").cgetContent("supervisorDispatch/viewPlanningPage");
   			//隐藏遮罩
			...
			serverSide:true,
            ajax: {
                url: 'supervisorDispatch/queryConstructionPlan',
                type:'post',
                data: function(d){
...
var searchPop = function(){
	$(".searchBtn").on("click",function(){
			var url = "#supervisorDispatch/suDispatchSearchPopPage";
			var popoptions = {
				title: '查询',
				content: url,
				accept: searchDone
