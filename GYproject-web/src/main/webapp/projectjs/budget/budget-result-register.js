    ...
    if ($('#budgetRegisterTable').length !== 0) {
    	budgetRegisterTable= $('#budgetRegisterTable').on( 'init.dt',function(){
    		$(this).bindDTSelected(trSelectedBack,true);
   			//加载页面
   			$("#budget_sum_box").cgetContent("budgetResultRegister/viewPage");
   			//隐藏遮罩
   			...
   			//查询费率		
   			setTimeout(function(){
//   			$("#budgetRegisterTable").DataTable().columns.adjust();
...
var  registerMonitor = function(){
	$(".registerBtn").on("click",function(){
		...
		$('.saveBtnBudget').removeClass("hidden");
	});
}
...
var quantitiesTabInit = function() {
    ...
    if (!$.fn.DataTable.isDataTable('#quanlitiesTable')) {
    	quantitiestable= $('#quanlitiesTable').on( 'init.dt',function(){
			...
			serverSide:true,
            ajax: {
                url: 'budgetResultRegister/queryCostSummary',
                type:'post',
                data: function(d){
...
function inportBtn(){
	...
	var popoptions = {
			title: '文件导入',
			content: "#budgetResultRegister/importPop?url=budgetResultRegister/importExcel",
			accept: importOk,
			atext: '完成',
			adisabled: true,
			nocache:true
