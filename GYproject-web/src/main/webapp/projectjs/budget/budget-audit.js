...
var handleBudgetAudit = function() {
    ...
    if ($('#budgetAuditTable').length !== 0) {
    	$('#budgetAuditTable').on( 'init.dt',function(){
   			//默认选中第一行
    		$(this).bindDTSelected(trSelectedBack,true);
   			//隐藏遮罩
   			$('#budgetAuditTable').hideMask();
   			$("#budgetAuditTable_filter input").attr("placeholder","工程编号");
   			//搜索监听
   			searchMonitor();   	
   			//跳转链接
   			if (crossPageBus) {
			...
			columnDefs: [{
				...
				"render": function ( data, type, row, meta ) {
					console.info(data);
					if(type==="display"){
						var html = getAuditLevelHtml(data,row.level,row.projId,"110306");						
						return html;
					}else{
						return data;
					}
...
var searchMonitor = function(){
	$(".searchBtn").off("click").on("click",function(){
		var url = "#budgetAudit/projectSearchPopPage";
		//加载弹屏
		$("body").cgetPopup({
			title: '查询',
			content: url,
	...
	//基础条件查询添加监听
	$('#budgetAuditTable_filter input').on('change',function(){
		var projNo = $('#budgetAuditTable_filter input').val();
		searchData = {};
		searchData.projNo = projNo;
		searchData.menuId = menuId;
		$("#budgetAuditTable").cgetData(false, function(){
   			//跳转到审核页面
			auditBtnMonitor();
		});
	});
	//基础条件查询添加回车事件
	$('#budgetAuditTable_filter input').on('keyup', function(event) {
	    if (event.keyCode == '13') {
	    	$(this).change();
	    }
	});
...
var searchDone = function(){
	searchData = $('#budgetAudit_searchForm').serializeJson();
	var projNo = $('#budgetAuditTable_filter input').val();
	searchData.projNo = projNo;
	searchData.menuId = menuId;
	//列表重新加载
...
var auditBtnMonitor = function(){
	$(document).off("click", ".level").on("click", ".level", function(){
		...
		$("#ajax-content").cgetContent("budgetAudit/auditPage",data);
	});
};
