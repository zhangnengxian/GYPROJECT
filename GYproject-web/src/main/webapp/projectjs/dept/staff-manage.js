	...
	return {
		...
		init : function() {
			if ($('#staffList').length !== 0) {
		    		...
		    		$(".edit").attr("data-c","#staff/viewPage");
		    		//地址详述区加载
...
var detailBack = function(data){
    searchData.corpId=data.corpId;
    searchData.staffId=data.staffId;
    searchData.belongCorpId=data.belongCorpId;
	...
	if(data.password && $("#loginStaffPost").val() &&  $("#loginStaffPost").val().indexOf('999')>0){
...
function onchangeSearch(value,event) {
	var arr = [];
	var selectedArr = [];

	var selectedData = $('#roleList').DataTable().rows('.selected').data();
	$.each(selectedData, function (key, val) {
		arr.push(val.roleId);
		selectedArr.push(val.roleId);
	});

	var data = $('#roleList').DataTable().data();
	$.each(data, function (key, val) {
		var roleId = val.roleId;
		var roleCode = val.roleCode;
		var roleName = val.roleName;
		if (roleName.search(value) !== -1 || roleCode.search(value) !== -1 || roleId.search(value) !== -1) {
			arr.push(roleId);
		}
	});

	if(event.keyCode===13){
		searchData.roleIds=arr.join(",");
		$("#roleList").cgetData(true,function () {
			selectedStaffRole(selectedArr);
		});
	}
}
