var projNoLength;
if(_inApk){
	projNoLength=12;
}else{
	projNoLength=15;
}
var corpId;
var projId;
var searchData = {};
var menuId="110616";
searchData.menuId = menuId;
var handleContractAudit = function() {
	"use strict";
	searchData.projNo=$(".sessionprojNo").val();
    if ($('#subContractAuditTable').length !== 0) {
    	$('#subContractAuditTable').on( 'init.dt',function(){
   			//默认选中第一行
    		$(this).bindDTSelected(trSelectedBack,false);
   			//隐藏遮罩
   			$('#subContractAuditTable').hideMask();
   			$("#subContractAuditTable_filter input").attr("placeholder","工程编号");
   			//搜索监听
   		
   			searchMonitor();
   			setTimeout(function(){
   			    $("#applyCompleteDrawingTable").DataTable().columns.adjust();
   			    //$("#workOrderTable").DataTable().responsive.recalc();
   			}, 0);
   		//跳转链接
   			if (crossPageBus) {
   				getSidebarMenu(11, true);
   				checkSidebarMenu(crossPageBus.hash)
   					//跳转后销毁对象
   				crossPageBus = null
   			}
        }).on( 'draw.dt', function () {
        	auditBtnMonitor();
        }).DataTable({
        	language: language_CN,
            lengthMenu: [18],
            dom: 'Bfrtip',
            buttons: [
                { text: '查询', className: 'btn-sm btn-query business-tool-btn searchBtn' }  
            ],
            //启用服务端模式，后台进行分段查询、排序
			serverSide:true,
            ajax: {
                url: 'scModifyAudit/querySubContract',
                type:'post',
                data: function(d){
                   	$.each(searchData,function(i,k){
                   		d[i] = k;
                   	});
                   	
                },
                dataSrc: 'data'
            },
            responsive: {
            	details: {
            		renderer: function ( api, rowIdx, columns ){
            			return renderChild(api, rowIdx, columns);
            		}
            	}
            },
            select: true,  //支持多选
            columns: [
				{"data":"scNo",responsivePriority:1},
				{"data":"projNo",responsivePriority:7},
				{"data":"projName",responsivePriority:3},
				{"data":"projectTypeDes",responsivePriority:4},
				{"data":"contributionModeDes",responsivePriority:5},
				{"data":"scAmount","className":"text-right",responsivePriority:6},
				{"data":"scSignDate",responsivePriority:7},
				{"data":"mrAuditLevel",responsivePriority:2},
				/*{"data":"overdue",className:"none never" }*/
			],
			columnDefs: [{
				"targets":5,
				"render":function(data,type,row,meta){
					if(data!=="" && data!==null){
						return parseFloat(data).toFixed(2);
					}else{
						return data;
					}
				}
			},{
				"targets": 7,
				"render": function ( data, type, row, meta ) {
					if(type==="display"){
						var html = getAuditLevelHtml(data,row.level,row.scId,menuId);
						return html;
					}else{
						return data;
					}
				},
			},{
				"targets":3,
				 "orderable":false
			},{
				"targets":4,
				 "orderable":false
			},{
				"targets":7,
				 "orderable":false
			},{
				"targets":2,
				//长字符串截取方法
				render: $.fn.dataTable.render.ellipsis({
					length: 10, 	//截取多少字符（或汉字）
					end: false	//如果是false从后部开始截取，及从后部开始计数，并裁掉超出的内容。如果是true从后部开始截取，及从后部开始计数，并裁掉超出的内容
				})
			},{
				"targets":0,
				//长字符串截取方法
				render: $.fn.dataTable.render.ellipsis({
					length: projNoLength, 	//截取多少字符（或汉字）
					end: true	//如果是false从后部开始截取，及从后部开始计数，并裁掉超出的内容。如果是true从后部开始截取，及从后部开始计数，并裁掉超出的内容
				})
			}]/*,
			fnRowCallback: function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
				if(aData.overdue){
					$(nRow).addClass("expired-proect");
				}
			}*/
        });
    }
};

/**
 * 弹屏监听方法
 */
var searchMonitor = function(){
	$(".searchBtn").off("click").on("click",function(){
		var url = "#scModifyAudit/projectSearchPopPage";
		var popoptions = {
			title: '查询',
			content: url,
			accept: searchDone
		};
		//加载弹屏
		$("body").cgetPopup(popoptions);
	});
	
	$("#subContractAuditTable_filter input").on("change",function(){
		searchData.projNo = $("#subContractAuditTable_filter input").val();
		searchData.menuId = menuId;
		//传入false  则不选中行
		$("#subContractAuditTable").cgetData(false, function(){
	   		//跳转到审核页面
			auditBtnMonitor();
		}); //列表重新加载
	});
	//基础条件查询添加回车事件
	$('#subContractAuditTable_filter input').on('keyup', function(event) {
		 if (event.keyCode == "13") {
			$(this).change();
		 }
    });
};

var searchDone = function(){
	
	searchData = $("#searchForm_sub").serializeJson();
	searchData.projNo = $("#subContractAuditTable_filter input").val();
	searchData.menuId = menuId;
    $("#subContractAuditTable").cgetData(false, function(){
   		//跳转到审核页面
    	auditBtnMonitor();
	});  //列表重新加载
    
}

/**
 * 加载审核屏
 */
var auditBtnMonitor = function(){
	$(document).off("click", ".level").on("click", ".level", function(){
		var isAudit = "0";//未审核过
		if($(this).is(".disabled")) return false;
		if($(this).is(".passed, .refused")){
			isAudit = "1";//已审核过
		}
		var currentLevel = $(this).attr("data-level");
		var scId = $(this).attr("data-rid");
		var data = {"scId":scId,"currentLevel":currentLevel,"isAudit":isAudit,"corpId":corpId,"projId":projId};
		$("#ajax-content").cgetContent("scModifyAudit/auditPage",data);
	});
	
};



/** 
 * 选中行时，查询详述
 */
var trSelectedBack = function(v, i, index, t, json){
	corpId=json.corpId;
    projId=json.projId;
}

/**
 * 初始化列表
 */
var contractAudit = function () {
	"use strict";
    return {
        //main function
        init: function () {
        	$.getScript("js/ellipsis.js").done(function(){
        		handleContractAudit();
        		
        	})
        }
    };
}();

//$(".attach-panel").initAttachOper({
//	collection: {
//		tableid: 'subContractAuditTable',
//	}
//});