...
var handleDrawingAudit = function() {
    ...
    if ($('#qualitiesAuditTable').length !== 0) {
    	qualitiesAuditTable =$('#qualitiesAuditTable').on( 'init.dt',function(){
			...
			serverSide:true,
            ajax: {
                url: 'budgetFirstAudit/queryProject',
                type:'post',
                data: function(d){
			...
			},
			{
				...
				"render": function ( data, type, row, meta ) {
					if(type==="display"){
						var html = getAuditLevelHtml(data,row.level,row.projId,"110619");
						return html;
					}else{
						return data;
					}
...
var searchMonitor = function(){
	$(".searchBtn").off("click").on("click",function(){
		var url = '#budgetFirstAudit/projectSearchPopPage';
		//加载弹屏
		$("body").cgetPopup({
			title: '查询',
			content: url,
...
var auditBtnMonitor = function(){
	$(document).off("click", ".level").on("click", ".level", function(){
		...
		$("#ajax-content").cgetContent("budgetFirstAudit/auditPage",data);
	});
};
