...
//              lengthMenu: [18],
	            dom: 'Brtip',
	            buttons: [
	                { text: '增加', className: 'btn-sm  btn-query business-tool-btn right-btn addBtn' },
	                { text: '删除', className: 'btn-sm  btn-warn business-tool-btn right-btn delBtn' },
	                { text: '导出', className: 'btn-sm  btn-default business-tool-btn right-btn exportBtn' },
	            ],
...
var qualitiesHistoryInit=function (){
	if ($('#auditHistoryTable').length !== 0) {
    	 $('#auditHistoryTable').on( 'init.dt',function(){
            ...
            serverSide:true,
            ajax: {
                url: 'qualitiesJudgement/queryManageRecord',
                type:'post',
                data: function(d){
