    ...
    if ($("#designAlterationAuditTable").length !== 0) {
    	var table = $("#designAlterationAuditTable").on( 'init.dt',function(){
           ...
           oLanguage: {   
			...
			serverSide:true,
            ajax: {
                url: 'engineeringQuantyAudit/queryVisaAudit',
                type:'post',
                data: function(d){
            ...
            columns: [
        		...
        		{"data":"projNo", responsivePriority: 3},
  	  			...
  	  			{"data":"quantitiesTotal", responsivePriority: 5,className:"none never"},
  	  			{"data":"evTypeDesc", responsivePriority: 6},
  	  			{"data":"visaDate", responsivePriority: 7},
...
var searchMonitor = function(){
	$(".searchBtn").off("click").on("click",function(){
		var url = "#engineeringQuantyAudit/searchPopPage";
		//加载弹屏
		$("body").cgetPopup({
			title: '查询',
			content: url,
...
var auditBtnMonitor = function(){
	$(document).off("click", ".level").on("click", ".level", function(){
		...
		$("#ajax-content").cgetContent("engineeringQuantyAudit/auditPage",data);
	});
