		...
		init : function() {
			if ($('#roleListTable').length !== 0) {
				roleManageTable = $('#roleListTable').on("init.dt",function(){
		    		...
		    		$(".edit").attr("data-c","#auth/viewPage");
		    		//详述区加载
		   			$("#role_manage_panel_box").cgetContent("auth/viewPage", "", function() {
		   				menuTree();
		   				deptTree();
		   			});
		   			$("#roleListTable_filter input").attr("placeholder","角色编号");
					$("#roleListPop").on("click",function(){
		   				var url = '#auth/roleSearchView';
		   				var popoptions = {
		   					title: '查询',
		   					content: url,
					...
					$('#roleListTable_filter input').on('change',function(){
						var roleCode = $('#roleListTable_filter input').val();
						roleManageSearchData = {};
						roleManageSearchData.roleCode = roleCode;
					...
					buttons : [ {
						text : '增加',
							className : 'btn-sm btn-query add business-tool-btn'
						},{
							text : '修改',
							className : 'btn-sm btn-query edit business-tool-btn'
						}, {
							text : '删除',
							className : 'btn-sm btn-warn del business-tool-btn'
						}
					],
					// 启用服务端模式，后台进行分段查询、排序
					serverSide : true,
...
var menuTree = function() {
    $('#menuTreeRoleManage').jstree({
    	...
    	if(selectedRowRoleManage >= 0) {
    		$("#roleListTable tr:eq(" + selectedRowRoleManage + ")").cgetDetail('roleManageform','auth/queryRole','',detailBackRoleManage); 
		}
    }); 
};
...
var trSelectedBackRoleManage = function(v, i, index, t, json){
	...
	//点击单行查看详情
	if(!isInitRoleManage) {
		t.cgetDetail('roleManageform','auth/queryRole','',detailBackRoleManage); 
	}
...
function deptChecked(deptIds) {
	$("#deptTree").jstree("uncheck_all");
	$("#deptTree").jstree("select_node", deptIds);  
}
...
var detailBackRoleManage = function(data){
	...
	menuChecked(menuIds);
	menuIdsRole = menuIds;
	detailBackRoleDept(data);
};
/**
 * 详述查询完回调函数
 * 受理部门
 * */
var detailBackRoleDept = function(data){
	$(".descrip").addClass("hidden");
	var ids = data.deptIds;
	var deptIds = ids!=null ? ids.split(',') : [];
	deptChecked(deptIds);
	deptIdsRole = deptIds;
};
...
$(document).off("click",".edit").on("click", ".edit", function(){
	...
	if(len>0){
		if($('#roleCode').removeClass('field-editable')){
			$('#roleCode').addClass('field-not-editable');
		}else{
			$('#roleCode').removeClass('field-editable')
			$('#roleCode').addClass('field-not-editable');
		}
		//切换到可编辑状态
		...
		if($(".corpId").val()==$(".groupCompany").val()){
			$("#corpId").removeAttr("disabled");
			$("#corpId").removeAttr("readonly");
			$('#corpId').removeClass('field-not-editable')
			$('#corpId').addClass('field-editable');
		}
		menuChecked(menuIdsRole);
		deptChecked(deptIdsRole);
	}else{
		var popoptions = {
			title: '提示',
			content: '请选择要修改的角色信息!',
...
var deleteDone=function(){
	$("#roleListTable").deleteRow(0,"auth/deleteRole",roleManageTable);
}
...
$(document).off("click",".saveBtn").on('click',".saveBtn",function(){
	...
	//$("#roleManageform").formSave("auth/addOrUpdateRole?menuIds="+menuIds,'roleListTable',roleManageTable,saveAddrBack);
	if($("#roleManageform").parsley().isValid()){
    	var data=$("#roleManageform").serializeJson();
    	$.ajax({
            type: 'POST',
            url: 'auth/addOrUpdateRole?menuIds='+menuIds+'&deptIds='+deptIds,
...
$(document).off("click",".add").on("click", ".add", function(){
	if($('#roleCode').removeClass('field-not-editable')){
		$('#roleCode').addClass('field-editable');
	}else{
		$('#roleCode').removeClass('field-not-editable')
		$('#roleCode').addClass('field-editable');
	}
	//切换到可编辑状态
	$("#roleManageform").toggleEditState(true);
