    ...
    <div class="row">
        ...
        <div class="panel panel-inverse">
                <div class="panel-heading">
                    ...
                    <h4 class="panel-title">合同评审列表</h4>
                </div>
                <div class="panel-body">
                	<input type="hidden" class="imgUrl"  value="${imgUrl}"/>
                	<input type="hidden" id="haveNotPrint" value="${alreadyPrint}"/>
					<input type="hidden" id="alreadyPrint" value="${haveNotPrint}"/>
                    <div class="mask-html text-center"><div><i class="fa fa-4x fa-spinner"></i><br><p class="text-ellipsis">加载中</p></div></div>
                    <table id="contractReviewPrintTable" data-attach-table="all" class="table table-striped table-bordered nowrap" width="100%">
                        <thead>
                            <tr>
                            	<th>crId</th>
                                <th>工程编号</th>
...
<script>
	...
	App.setPageTitle('合同评审打印 - 工程管理系统');
	...
	$.getScript('projectjs/contract/contract-review-print.js?'+Math.random()).done(function () {
    ...
    var showReport = function(){
    	...
    	var crId='-1',
    		...
    		projName='';
    	if(trSData.json){
    		crId = encodeURIComponent(cjkEncode(trSData.json.crId));
    		projId = encodeURIComponent(cjkEncode(trSData.json.projId));
    		projName = encodeURIComponent(cjkEncode(trSData.json.projName));
    	}
    	imgUrl = $(".imgUrl").val();
    	src = cptPath+"/ReportServer?reportlet=contract/"+cptType+"&crId=" + crId + "&projId=" + projId + "&projName=" + projName;
    	src = src + "&imgUrl="+imgUrl+"&crId="+crId;
