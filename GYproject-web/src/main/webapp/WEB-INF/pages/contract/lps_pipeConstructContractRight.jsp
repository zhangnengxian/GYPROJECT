...
//     }
	...
	$("#payType").change(function(){
		//付款方式1，2，3
		var payTypeMode = $("#payType").find("option:selected").attr("data-mode");
		$("input[name='isRatio']:eq(0)").attr("checked",'checked'); //默认选中
        if(payTypeMode=="1"){
        	//首付款录入
        	//$('.firstPayment').show();
            $(".payPlat").addClass("hidden");
  ...
  //选择申报单位
    ...
    $('[name="isRatio"]').on("change", function () {
        if ($('[name="isRatio"]:checked').val() == 0) {   //不按比例方式付款
            ...
            $("#firstPayment").addClass("field-not-editable");
            $("#secondPayment").addClass("field-not-editable");
            $("#thirdPayment").addClass("field-not-editable");
            $("#paymentRatio1").removeClass("field-not-editable");
            $("#paymentRatio2").removeClass("field-not-editable");
            $("#paymentRatio3").removeClass("field-not-editable");
