    ...
    <div class="clearboth form-box">
    	...
    	<form class="form-horizontal" id="contractDetailform"  data-parsley-validate="true" action="">
			...
			<div class="form-group col-md-6 col-sm-12 col-sm-6">
				...
				<div>
					<label>
						<input type="radio" class="field-editable" name="isSpecial" value="1" /> 是
					</label>
					<label>
						<input type="radio" class="field-editable" name="isSpecial" value="0" checked/> 否
					</label>
				</div>
			</div>
...
<script>
    ...
    //审核功能表单回显
    var contractStr = '${contractJsonStr}'.replace(/[\r\n]/g, "");
    if (contractStr != null && contractStr != '') {
        var jsonObj = eval('(' + contractStr + ')');
    ...
    function dataBack(data) {
        ...
        $.ajax({
            type: 'post',
            url: 'constructContract/queryPayType?id=' + sedata + "&corpId=" + data.corpId,
            dataType: 'json',
...
// 	defaultValue();
    ...
    //如果是历史数据则根据付款金额显示付款比例
    function historyDataDeal() {
        if (($("#firstPayment").val() != null || $("#firstPayment").val() != "") && ($("#paymentRatio1").val() != null || $("#paymentRatio1").val() != "")) {
            $("#paymentRatio1").val((new Number($("#firstPayment").val()) / new Number($("#contractAmount").val())).toFixed(2) * 100.00);
        }
        if (($("#secondPayment").val() != null || $("#secondPayment").val() != "") && ($("#paymentRatio2").val() != null || $("#paymentRatio2").val() != "")) {
            $("#paymentRatio2").val((new Number($("#secondPayment").val()) / new Number($("#contractAmount").val())).toFixed(2) * 100.00);
        }
        if (($("#thirdPayment").val() != null || $("#thirdPayment").val() != "") && ($("#paymentRatio3").val() != null || $("#paymentRatio3").val() != "")) {
            $("#paymentRatio3").val((new Number($("#thirdPayment").val()) / new Number($("#contractAmount").val())).toFixed(2) * 100.00);
        }  //如果是历史数据则根据付款金额显示付款比例
    }
...
// 	}
    ...
    $(".saveBtn").on("click", function () {
        ...
        if ($("#contractDetailform").parsley().isValid()) {
            ...
            var mdata = tableform.getDTFormData();
            var resultData =getScaleData(data.projectType);
...
//                		$("#contractDetailform input,#contractDetailform textarea,#contractDetailform select").val('');
    ...
    $('[name="isRatio"]').on("change", function () {
        if ($('[name="isRatio"]:checked').val() == 0) {   //不按比例方式付款
            $("#firstPayment").removeClass("field-not-editable");
            $("#secondPayment").removeClass("field-not-editable");
    ...
    $("#contractType").change(function () {
        ...
        $.ajax({
            type: 'post',
            url: 'constructContract/queryPayType?id=' + data + "&corpId=" + $("#corpId").val(),
            dataType: 'json',
            success: function (data) {
                $("#payType").empty();
                $("#payType").append('<option value=""></option>');
    ...
    /* var loseBlur=function(paymentRate){
        var reg=/^[0-9]+\.[0]+$/; //正则表达式,验证是否是整数
        var payNumber = $("#payType").val();    //分几期付款，1、7、3代表全款，6、2、4代表分两期，5代表分三期
       if(((new Number(paymentRate))>100 || !reg.test(paymentRate))  && payNumber!='6'  ){  //判断是是否在1到100之间且为整数
             $("body").cgetPopup({
                 title: '提示',
                 content: '请输入1到100之间的整数!',
                 ahide:true,
        ...
        if (payNumber == '4' || payNumber == '6' || payNumber == '2') {  //分两期付款
            ...
            if (paymentRatio1 > 0 && paymentRatio2 > 0) {
                if ((paymentRatio1 + paymentRatio2) <= 100.00) {  //判断前两期首付比例是否小于或者等于100
                    ...
                    $("#paymentRatio3").val((100.00 - paymentRatio1 - paymentRatio2).toFixed(2));    //计算第三次付款的比列
    ...
    /*     $("#secondPayment").on("change",function(){
            if($("#payType").val()=="2"||$("#payType").val()=="4"||$("#payType").val()=="6"){
