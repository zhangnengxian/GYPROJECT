...
<div class="infodetails">
	...
	<div class="clearboth form-box">
		...
		<form class="form-horizontal" id="contractDetailform"  data-parsley-validate="true" action="">
			...
			<div class="form-group col-md-6 col-sm-12 col-sm-6  clearboth">
				...
				<div>
					<select class="form-control input-sm field-editable" id="payType"  name="payType"  data-parsley-required="true">
						...
						<option value="1102-201" data-mode="1">一次性付清</option>
						<option value="1102-202" data-mode="2">两次付清</option>
						<option value="1102-208" data-mode="3">三性付清</option>
						<option value="1102-204" data-mode="4">四次付清</option>
					</select>
				</div>
			</div>
			<br/>
			...
			<div class="form-group col-md-6 agentCustPhone  hidden ">
				<label class="control-label" for="agentCustPhone">阶段款比列</label>
				<div>
					<input type="text" class=" form-control input-sm fixed-number text-right" id="agentCustPhone" readonly="readonly" name="agentCustPhone" data-parsley-type="number"
						   data-parsley-maxlength="6" value="" data-parsley-required="true" step="1" min="0" max="100" placeholder="请输入付款比例"  onblur="loseBlur(this.value)">
					<a href="javascript:;" class="btn btn-sm btn-default selectDisabled disabled">%</a>
				</div>
			</div>
			<div class="form-group col-md-6 agentUnitAddress hidden">
				<label class="control-label" for="agentUnitAddress">阶段款</label>
				<div>
					<input type="text" class=" form-control input-sm field-not-editable fixed-number text-right" id="agentUnitAddress"  name="agentUnitAddress" data-parsley-type="number" data-parsley-maxlength="16" value="" data-parsley-required="true">
					<a href="javascript:;" class="btn btn-sm btn-default">元</a>
				</div>
			</div>
...
<script>
    function showPayTypeInfo(){
        ...
        if(payTypeMode=="1"){
            ...
            $(".agentCustPhone").addClass("hidden");
            $(".agentUnitAddress").addClass("hidden");
            ...
            $(".paymentRatio2").removeClass("hidden");
            $(".paymentRatio1").removeClass("hidden");
            $(".agentCustPhone").removeClass("hidden");
            $('.firstPayment').removeClass("hidden");
            $('.secondPayment').removeClass("hidden");
            $('.thirdPayment').removeClass("hidden");
            $('.thirdPayment').removeClass("hidden");
            $('.agentUnitAddress').removeClass("hidden");
        }
    ...
    function dataBack(data){
        ...
        $.ajax({
            type: 'post',
            url: 'constructContract/queryPayType?id='+sedata+"&corpId=1102",
            dataType: 'json',
...
//				$("#payType").append('<option value=""></option>');
                ...
                /*if($("#corpId").val()=="1102" && $("#conId").val()==''){
                    $("#payType").empty();
                    $("#payType").append('<option value='+"1102-201"+' selected data-mode="'+"1"+'">' + "一次性付清"+ '</option>');
                    $("#payType").append('<option value='+"1102-202"+' selected data-mode="'+"2"+'">' + "二次付清"+ '</option>');
                    $("#payType").append('<option value='+"1102-208"+' selected data-mode="'+"3"+'">' + "三次付清"+ '</option>');
                    $("#payType").append('<option value='+"1102-204"+' selected data-mode="'+"4"+'">' + "四次付清"+ '</option>');
                    $("#payType").get(0).value = payType;
                    $("#payType").change();
                }*/
            },
            error: function(data){
...
//                		$("#contractDetailform input,#contractDetailform textarea,#contractDetailform select").val('');
    ...
    var  CalculationFunction=function(){
        ...
        }else if(payTypeMode=='4'){  //分四次
            $("#paymentRatio1").val('30.00');    //计算第一次付款的比列
            $("#firstPayment").val((totaAmount*(30/100.00)).toFixed(2));  //第一次多少钱
            $("#paymentRatio2").val('40.00');    //计算第二次付款的比列
            $("#secondPayment").val((totaAmount*(40/100.00)).toFixed(2));  //第二次多少钱
            $("#paymentRatio3").val('25.00');    //计算第三次付款的比列
            $("#thirdPayment").val((totaAmount*(25/100.00)).toFixed(2));  //第三次付多少钱
            $("#agentCustPhone").val('5.00');    //计算第四次付款的比列
            $("#agentUnitAddress").val((totaAmount*(5/100.00)).toFixed(2));  //第四次付多少钱
        }
    }
