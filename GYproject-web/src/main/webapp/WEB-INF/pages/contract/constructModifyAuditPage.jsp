...
<script>
  ...
  $.getScript('projectjs/contract/contract-modify-audit-page.js?'+Math.random()).done(function () {
        auditHistory.init();
	});

    //放弃
    $(".auditCancelBtn").off("click").on("click",function(){
		$("#ajax-content").cgetContent("contractModifyAudit/main");
	});
    ...
    $(".auditSaveBtn").off("click").on("click",function(){
    	...
    	if(val==null){
        	...
        	if($("#contractAuditRightForm").parsley().isValid()){
            	...
            	$.ajax({
                    type: 'POST',
                    url: 'contractModifyAudit/auditSave',
