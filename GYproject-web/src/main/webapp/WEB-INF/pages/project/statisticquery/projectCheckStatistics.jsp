			...
			<div class="panel panel-inverse">
				...
				<div class="panel-body">
					...
					<form class="" id="projectConditionForm" action="/" method="POST">
						...
						<div class="form-group">
							<div>
								<select type="text" class="form-control input-sm select-filter" name="checkType">
									<option value="">验收类型</option>
									<option value="1">联合验收</option>
									<option value="2">分布验收</option>
								</select>
							</div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
				            <div>
				                <input type="text" class="form-control input-sm datepicker-default projectAcceptData" placeholder="验收起始日期" id="jaDateStart" name="jaDateStart" value=""/>
				            </div>
				      	</div>
				      	<div class="form-group">
				            <div>
				                <input type="text" class="form-control input-sm datepicker-default projectAcceptData" placeholder="验收截止日期" id="jaDateEnd" name="jaDateEnd" value=""/>
				            </div>
				      	</div>
					</form>
...
<script>
	...
	App.setPageTitle('报表查询 - 工程验收统计报表 ');
   ...
   var conditions = function(){
	  ...
	  //工程编号
	  if(formData.projNo !== ''){
		  sql=sql+" and proj_no like '%"+formData.projNo+"%'";
	  }
       //公司ID
       if($('#corpId').val() != ''){
           sql=sql+" and corp_id like '"+$('#corpId').val()+"%'";
       }
	  //工程名称
	  if(formData.projName !== ''){
		  sql = sql+" and proj_name like'%"+formData.projName+"%'";
	  }
       //工程类型
       if(formData.projectType !== ''){
           sql = sql+" and project_type ='"+formData.projectType+"'";
       }
       //验收类型
       if(formData.checkType !== ''){
           sql = sql+"and checkType ='"+formData.checkType+"'";
       }
       //出资方式
	  ...
	  if(formData.cuNameId !== ''){
		  sql = sql+" and cu_id ='"+formData.cuNameId+"'";
	  }
	  //现场代表
	  if(formData.builder !== ''){
		  sql = sql+" and builder like '%"+formData.builder+"%'";
	  }
	  //验收开始日期
	  if(formData.jaDateStart !== ''){
		  sql = sql+" and jaDate >= "+sqlDateConveter("mysql",formData.jaDateStart,1);
	  }
	  //验收开始日期
	  if(formData.jaDateEnd !== ''){
		  sql = sql+" and jaDate <= "+sqlDateConveter("mysql",formData.jaDateEnd,1);
	  }
	  return sql;
   }
   //加载打印预览
   var showReport = function(){
   	...
   	src="<%=basePath%>/ReportServer?reportlet=statisticalquery/projectCheckStatistics.cpt&condition="+condition+"&staffName="+staffName;
