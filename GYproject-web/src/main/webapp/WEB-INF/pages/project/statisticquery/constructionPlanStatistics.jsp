			...
			<div class="panel panel-inverse">
				...
				<div class="panel-body">
					...
					<form class="" id="projectConditionForm" action="/" method="POST">
						...
						<div class="form-group">
				            <div>
				                <input type="text" class="form-control input-sm datepicker-default projectAcceptData" placeholder="计划下达起始日期" id="acceptDateStart" name="acceptDateStart" value=""/>
				            </div>
				      	</div>
				      	<div class="form-group">
				            <div>
				                <input type="text" class="form-control input-sm datepicker-default projectAcceptData" placeholder="计划下达截止日期" id="acceptDateEnd" name="acceptDateEnd" value=""/>
				            </div>
				      	</div>
					</form>
...
<script>
   ...
   var conditions = function(){
	  ...
	  if($("#unitType").val()=='1'){
		  ...
		  sql=sql+" and c.su_id ='"+$("#loginDeptId").val()+"'";
	  }else if($("#unitType").val()=='5'){
		  sql=sql+" and c.cu_id='"+$("#loginDeptId").val()+"'";
	  }
	 //工程编号
	  if(formData.projNo !== ''){
	  ...
	  if(formData.acceptDateStart !== ''){
		 // sql = sql+" and c.CP_ARRIVE_DATE >= to_date('"+formData.acceptDateStart+"','YYYY-MM-DD')";
		  sql = sql+" and c.CP_ARRIVE_DATE >= "+sqlDateConveter("mysql",formData.acceptDateStart,1);
	  }
	  //计划下达结束日期
   ...
   var showReport = function(){
   	...
   	src="<%=basePath%>/ReportServer?reportlet=statisticalquery/constructionPlanStatistics.cpt&condition="+condition+"&staffName="+staffName+"&acceptDateStart="+acceptDateStart+"&acceptDateEnd="+acceptDateEnd+"&unitType="+$("#unitType").val()+"&deptId="+$("#loginDeptId").val();
