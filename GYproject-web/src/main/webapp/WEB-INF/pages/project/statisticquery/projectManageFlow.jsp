...
<script>
   ...
   var conditions = function(){
	  ...
	  if($("#unitType").val()=='1' && $("#deptId").val()!==''){
		  ...
		  sql=sql+" and pl.su_id ='"+$("#deptId").val()+"'";
	  }else if($("#unitType").val()=='5'){
		  sql=sql+" and pl.cu_id='"+$("#deptId").val()+"'";
	  }
	  //工程编号
	  if(formData.projNo !== ''){
	...
	//工程类型
      ...
      if(formData.projStatusId !== '' && formData.projStatusIdEnd == ''){
    	  ...
    	  sql = sql+" and pro.PROJ_STATUS_ID = '"+formData.projStatusIdEnd+"'";
      }else if(formData.projStatusId !== '' && formData.projStatusIdEnd != ''){
    	  sql = sql+" and pro.PROJ_STATUS_ID  between '"+formData.projStatusId+"' and '" +formData.projStatusIdEnd+"'";
      }
	  //分包公司
	  if(formData.cuNameId !== ''){
		  sql = sql+" and pl.cu_id ='"+formData.cuNameId+"'";
	  }
	  //监理公司
	  if(formData.suId !== ''){
		  sql = sql+" and pl.su_id ='"+formData.suId+"'";
	  }
	  //甲方代表
	  if(formData.builder !== ''){
		  sql = sql+" and pl.builder like '%"+formData.builder+"%'";
	  }
 	  //立项开始日期
   ...
   var showReport = function(){
   	...
   	src="<%=basePath%>/ReportServer?reportlet=statisticalquery/projectManageFlow1.cpt&__bypagesize__=false&condition="+condition+"&staffName="+staffName;
