				...
				<div class="panel-body">
					...
					<form class="" id="projectConditionForm" action="/" method="POST">
						...
						<div class="form-group">
				      	...
				      	<div class="form-group">
				            <div>
				                <input type="text" class="form-control input-sm" placeholder="现场负责人" name="fieldPrincipal" value=""/>
				            </div>
				      	</div>
						<div class="form-group">
				            <div>
				                <input type="text" class="form-control input-sm datepicker-default " placeholder="检查开始日期" name="checkDateStart" value=""/>
				            </div>
				      	</div>
				      	<div class="form-group">
				            <div>
				                <input type="text" class="form-control input-sm datepicker-default " placeholder="检查截止日期" name="checkDateEnd" value=""/>
				            </div>
				      	</div>
					</form>
...
<script>
   ...
   var conditions = function(){
	  ...
	  //工程编号
	  if(formData.projNo !== ''){
		  sql=sql+" and i.proj_no='"+formData.projNo+"'";
	  }
       //公司ID
       if($('#corpId').val() != ''){
	  ...
	  if(formData.projName !== ''){
		  sql = sql+" and i.proj_name like'%"+formData.projName+"%'";
	  }
	  //工程类型
	  ...
	  //现场负责人
	  if(formData.fieldPrincipal !== ''){
		  sql = sql+" and i.field_principal like '%"+formData.fieldPrincipal+"%'";
	  }
	  //检查开始日期
	  if(formData.checkDateStart !== ''){
		  sql = sql+" and i.check_date>="+sqlDateConveter("mysql",formData.checkDateStart,1);
	  }
	  //检查结束日期
	  if(formData.checkDateEnd !== ''){
		  sql = sql+" and i.check_date<="+sqlDateConveter("mysql",formData.checkDateEnd,1);
	  }
	  return sql;
   }
   //加载打印预览
   var showReport = function(){
   	...
   	src="<%=basePath%>/ReportServer?reportlet=statisticalquery/queryInspectionList.cpt&condition="+condition+"&staffName="+staffName;
