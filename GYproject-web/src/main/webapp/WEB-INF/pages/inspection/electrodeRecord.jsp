						    ...
						    <div class="clearboth form-box">
								...
								<form class="form-horizontal" id="signForm" data-parsley-validate="true" action="electrodeRecord/saveSign">							
									...
									<div class="form-group col-md-6 col-sm-12">
										...
										<div>
											<input type="text" class="form-control input-sm field-not-editable addClear"  id="inspectionDate" name="inspectionDate"   />
										</div>
									</div>
						    		<div class="form-group col-sm-12">
				                    	<label class="control-label" for="inspectionResult">查验结果</label>
					                    <div> 
				                        	<textarea class="form-control field-editable addClear allText welder" name="inspectionResult" id="inspectionResult" rows="2" cols="" data-parsley-maxlength="200" ></textarea>
					                    </div>
					  			    </div> 
								    ...
								    <div class="form-group col-md-6 col-sm-12 allSign welder">
										<label class="control-label signature-tool sign-require sign-all" for="recorder">记录人</label>
										<div>
								    ...
								    <div class="form-group col-md-6 col-sm-12 hidden selectSignDate">
										...
										<div>
											<input type="text" class="form-control input-sm datepicker-default field-editable addClear allText welder" value="" id="recorderDate" name="recorderDate"  />
										</div>
									</div>
									<div class="form-group col-md-6 col-sm-12 allSign construction">
										<label class="control-label signature-tool sign-require sign-all" for="reviewer">复核人</label>
										<div>
						    ...
						    <div class="clearboth form-box">
						    	...
						    	<input type="hidden" id="flag1"  class="addClear">
									<form class="form-horizontal" id="auditForm" data-parsley-validate="true" action="${actionName }/saveRecord">
		                   			<input type="hidden" id="erId" name="erId" class="addClear">
									<input type="hidden" id="version1" name="version" class="addClear"/>
		                   			...
		                   			<div class="form-group col-md-6 col-sm-12">
										<label class="control-label" for="electrodeNo">焊条牌号</label>
										<div>
							                <input type="text" class="form-control field-editable input-sm addClear allText welder" id="electrodeNo"   name="electrodeNo"  data-parsley-required="true" data-parsley-maxlength="30"/>
							            </div>
									</div>
		                   			<div class="form-group col-md-6 col-sm-12">
										<label class="control-label" for="electrodeForamt">焊条规格</label>
										<div>
							                <input type="text" class="form-control field-editable input-sm addClear allText welder"  id="electrodeForamt"   name="electrodeForamt"  data-parsley-maxlength="50"/>
							            </div>
									</div>
		                   			<div class="form-group col-md-6 col-sm-12">
										<label class="control-label" for="recipientNum">领用数量</label>
										<div>
							                <input type="text" class="form-control input-sm field-editable addClear allText welder" id="recipientNum"   name="recipientNum"  data-parsley-maxlength="50"/>
							            </div>
									</div>
							        ...
							        <div class="form-group col-md-6 col-sm-12">
							            ...
							            <div class="input-group">
								            <input type="text" class="form-control input-sm datepicker-default addClear field-not-editable sysDate"  id="recipientDate" name="recipientDate"  data-parsley-required="true"  value="">
								        </div>
							        </div>
							       <div class="form-group col-md-6 col-sm-12">
										<label class="control-label" for="ecBatchNum">编号/批号</label>
										<div>
							                <input type="text" class="form-control input-sm field-editable addClear allText welder" id="ecBatchNum"   name="ecBatchNum"  data-parsley-required="true" data-parsley-maxlength="30"/>
							            </div>
									</div>
									<div class="form-group col-md-6 col-sm-12 allSign welder">
						            	<label class="control-label signature-tool sign-require" for="recipientStaff">领用人</label>
										<div>
											<a href="javascript:;" class="btn btn-sm btn-white" id="signBtn_3"><i class="fa fa-pencil"></i></a>
											<input type="hidden" class="sign-data-input" id="recipientStaff" name="recipientStaff" value="">
											<input type="hidden" class="signPost"  id="recipientStaff_postType" name="recipientStaff_postType" value="${reviewerPost }">
											<img class="recipientStaff" alt="" src="" style="height: 30px">
											<span class="clear-sign"><i class="fa ion-close-circled"></i></span>
										</div>
							        </div>
		                       	...
		                       	<div class="clearboth form-box  photoBox">
									<div class="form-group width-full attach-images-list hidden" id="recordImagesList">
										<input type="hidden" id="erId1" class="addClear"/>
...
<script>
    ...
    App.setPageTitle('焊条领用 - 工程项目管理系统 ');
  ...
  //签字加载方式
   ...
   /* //签字区保存功能
    $(".savebtn").on("click",function(){
    	$("#signForm").cformSave("electrodeRecordTable",saveBack,false);
    });
    
    function saveBack(data){
    	$('.update-show').addClass("hidden");
    	$('#electrodeRecordForm').toggleEditState(false);
    	//保存签字信息
    ...
    $.getScript('projectjs/inspection/inspection-common.js?'+Math.random()).done(function() {
		 $.getScript('projectjs/inspection/electrode-record.js?'+Math.random()).done(function () { 
		   	electrodeRecord.init();
	 	}); 
    });
    ...
    var showReport = function(){
        ...
        //cpt路径及参数
    	var src=cptPath+"/ReportServer?reportlet=inspection/electrodeRecord.cpt";
  ...
  //移动端点击上传
    $("#projectImagesList .uploadBtn").off("click").on("click",function(){
		...
		if(_inApk && $(".attach-images-list").find(".new-image").length){
		    ...
		    if (files.length) {
		    	uploadImages(files, 0, files.length, projId, projNo, stepId, busRecordId,"1","2");
		    } else {
