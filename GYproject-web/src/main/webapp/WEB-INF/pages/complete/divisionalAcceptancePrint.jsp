    ...
    <div class="row">
        ...
        <div class="panel panel-inverse">
                <div class="panel-heading">
                    ...
                    <h4 class="panel-title">分部验收列表</h4>
                </div>
...
<script>
	...
	$.getScript('projectjs/complete/divisional-accept-print.js?'+Math.random()).done(function () {
		projectSelfCheckPrint.init();
	});

    function cjkEncode(D){if(typeof D!=="string"){return D}var C="";for(var A=0;A<D.length;A++){var B=D.charCodeAt(A);if(B>=128||B==91||B==93){C+="["+B.toString(16)+"]"}else{C+=D.charAt(A)}}return C}
    var showReport = function(){
    	var daId=$("#daId").val(),projId=$("#projId").val(),imgUrl = $(".imgUrl").val();
    	//imgUrl = encodeURIComponent(cjkEncode(imgUrl));
    	
    	console.info("imgUrl--"+imgUrl);
    	src = cptPath+"/ReportServer?reportlet=complete/"+cptType+"&daId=" + daId + "&projId=" + projId+"&imgUrl="+imgUrl;
