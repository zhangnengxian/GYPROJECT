        ...
        <div class="col-sm-6 col-xs-12">
            ...
            <div class="panel panel-inverse">
                <div class="panel-heading">
                   ...
                   <h4 class="panel-title">竣工图审批申请</h4>
                </div>
                <div class="panel-body" id="audit_complete_drawing_panelBox">
                    <!-- 详述 -->
            	</div>
        	</div>
        </div>
            ...
            <div class="panel panel-inverse" id="content">
			    ...
			    <div class="panel-body" id="delay_check_panel_box">
			    	...
			    	<div class="clearboth form-box checkFormDiv">
			    		<form class="form-horizontal" id="auditRightForm" action="">
			    			<input type="hidden" class="isAudit" id="isAudit" name = "isAudit" value = "${ isAudit}"/>
			    			<input type="hidden" class="projId" id="projId" name = "projId" value = "${da.projId}"/>
			    			<input type="hidden"  class="projNo" id="projNo" name = "projNo" value = "${da.projNo}"/>
                    		<input type="hidden" id="businessOrderId" name = "businessOrderId" value = "${da.acdId}"/>
...
<script>
    ...
    App.setPageTitle('竣工图审核 - 工程管理系统');
    $("#auditCompleteDrawingDetailForm").styleFit();
    $("#auditRightForm").styleFit();
    	
    //表单不可编辑
    $("#auditCompleteDrawingDetailForm,#auditRightForm").toggleEditState(false);
    $("#auditRightForm").toggleEditState(true);
    ...
    $.getScript('projectjs/settlement/audit-complete-drawing.js?v='+Math.random()).done(function () {
        auditHistory.init();
	}); 
    
     //放弃
    $(".checkCancelBtn").off("click").on("click",function(){
		$("#ajax-content").cgetContent("auditCompleteDrawing/main");
	});
    ...
    $(".checkSaveBtn").off("click").on("click",function(){
    	var val=$('#auditRightForm input:radio[name="mrResult"]:checked').val();
    	if(val==null){
    		alertInfo("请选择审批结果！");
    	}else{
    		$(".projId").val($("#projId").val());
        	$(".projNo").val($("#projNo").val());
        	$("#auditRightForm").cformSave("auditHistoryTable",auditSaveCallBack,'','','',$(".checkSaveBtn"));
        	if($("#auditRightForm").parsley().isValid()){
            	var data=$("#auditRightForm").serializeJson();
            	$.ajax({
                    type: 'POST',
                    url: 'auditCompleteDrawing/auditSave',
                    ...
                    success: function (data) {
                    	...
                    	if(data === "false"){
                    		...
                    		$("#auditRightForm").toggleEditState(false);
            	...
            	//如果通过验证, 则移除验证UI
            	$("#auditRightForm").parsley().reset();
            }else{
            	//如果未通过验证, 则加载验证UI
            	$("#auditRightForm").parsley().validate();
            }
    	}
    });
