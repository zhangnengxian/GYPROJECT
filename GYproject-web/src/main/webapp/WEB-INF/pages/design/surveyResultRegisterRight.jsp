...
function surveyBack(data){
	...
	if($("#post").val()!=""){
		...
		if($("#post").val().indexOf($("#designerPost").val())>=0){
			...
			/* $("#custName").removeClass("field-not-editable");
			$("#custName").addClass("field-editable");
			$("#projAddr").removeClass("field-not-editable");
			$("#projAddr").addClass("field-editable"); */
		}
	}else{
	...
	if($("#custName").val()==""){
	 ...
	 if($("#projName").val()==''){
		 ...
		 $("#projAddr").attr("readonly",false);
		 $("#projAddr").removeAttr("disabled"); */
	 }
    ...
    var backDone2 = function(o){
		...
		$(".infodetails").loadMask("正在保存...", 1, 0.5);
      	...
      	viewdata.scaleDetails = resultData;
       	...
       	var surveyDesDate=$('#surveyDesDate').val();//踏勘结果日期
		if (surveyDesDate!=''){
			viewdata.surveyDate=$('#surveyDesDate').val();//保存时：踏勘日期为第一次探勘结果日期
		}
