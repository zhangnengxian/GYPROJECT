...
public class FileSteamUpload extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		MenuDao menuDao=(MenuDao)ac2.getBean("menuDaoImpl");
		Menu menu = menuDao.get(request.getParameter("menuId"));
		String stepDes=menu.getMenuName();
		...
		while (flag == true) {
			...
			case DATAHEADER:
				...
				if (pos == -1) {
					...
					fileRealName = s;
					//fileRealName = new String(fileRealName.getBytes("ISO-8859-1"),"UTF-8");
					...
					if (fileRealName.length() != 0) { // 确定有文件被上传
						...
						path1=Constants.FIRST_DISK_PATH+request.getParameter("projNo")+"/"+stepDes;
						String dirPath = path+path1;
		...
		acc.setAlPath(path1+"/"+fileName);
		//文件名称没有后缀
		//String name1=fileName.substring(0,fileName.lastIndexOf("."))
		String fileType=fileName.substring(fileName.lastIndexOf(".")+1,fileName.length());	
		acc.setAlName(name);
		acc.setProjId(request.getParameter("projId"));
		acc.setProjNo(request.getParameter("projNo"));
		acc.setStepId(stepDes);
		...
		AccessoryDao accessoryDao=(AccessoryDao)ac2.getBean("accessoryDaoImpl");
