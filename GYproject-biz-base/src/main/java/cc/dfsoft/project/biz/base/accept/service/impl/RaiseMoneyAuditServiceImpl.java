 ...
 * @ClassDesc: 提资审核实现类
 * @author: zhangnx
 * @date: 10:40 2018/9/14
 * @version: V1.0
 */
@Service
@Transactional(readOnly = true, propagation = Propagation.REQUIRED)
public class RaiseMoneyAuditServiceImpl implements RaiseMoneyAuditService {
    private static Logger logger = LoggerFactory.getLogger(RaiseMoneyAuditServiceImpl.class);
    ...
    /**
     * @methodDesc: 查询待提资审核工程
     * @author: zhangnx
     * @date: 10:41 2018/9/14
     */
    @SuppressWarnings("unchecked")
    @Override
    public Map<String, Object> getAuditProject(ProjectQueryReq req) throws ParseException {
        String grade=null;
        ...
        if(data!=null && data.size()>0){
            ...
            for(int i = 0;i<data.size();i++){
                ...
                //分公司查询审核级别
                DocType docType = docTypeService.findByStepId(StepEnum.RAISEMONEY_AUDIT.getValue(),data.get(i).getCorpId(),pro.getProjectType(),pro.getContributionMode());
                ...
                mrq.setStepId(StepEnum.RAISEMONEY_AUDIT.getValue());
                mrq.setProjId(data.get(i).getProjId());
                //审核信息
                List<ManageRecord> mrls = managerecorddao.findByStepIdProjIdIsPass(data.get(i).getProjId(),StepEnum.RAISEMONEY_AUDIT.getValue(), MrResultEnum.PASSED.getValue());
     ...
     * @methodDesc:查询工程详细
     * @author: zhangnx
     * @date: 10:42 2018/9/14
     */
    @Override
    public Project getProjectById(String projId) {
        Project project = projectDao.get(projId);

        RaiseMoney raiseMoney = raiseMoneyDao.get("projId",projId);
        if (raiseMoney!=null){
            project.setRemark(raiseMoney.getRemark());
            project.setUploadFlag(raiseMoney.getUploadFlag());
        }
        return project;
    }
