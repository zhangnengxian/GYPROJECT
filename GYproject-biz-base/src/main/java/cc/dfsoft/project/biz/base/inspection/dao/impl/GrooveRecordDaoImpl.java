...
@Repository
public class GrooveRecordDaoImpl extends NewBaseDAO<GrooveRecord, String> implements  GrooveRecordDao {
	...
	public Map<String, Object> queryGrooveRecord(GrooveRecordReq dtoReq) {
		Criteria c = super.getCriteria();			
		 ...
		 // 根据条件获取查询信息
		 List<GrooveRecord> list = this.findBySortCriteria(c, dtoReq);
	...
	public void deleteByPcId(String pcId) {
		StringBuffer hql = new StringBuffer("delete from GrooveRecord where pcId='").append(pcId).append("'");
	...
	public Map<String, Object> queryRecords(GrooveRecordReq grReq) {
		...
		int filterCount = this.countByCriteria(c);
		// 根据条件获取查询信息
		List<GrooveRecord> list = this.findBySortCriteria(c, grReq);
		// 返回结果
	...
	public void updatePcIdByRecordId(String id, String pcId) {
		StringBuffer hql = new StringBuffer("update  GrooveRecord set pcId='").append(pcId).append("' where grId='").append(id).append("'");
		this.executeHql(hql.toString());
	}

	@Override
	public void updateByPcId(String pcId) {
		StringBuffer hql  =  new StringBuffer("update  GrooveRecord set pcId=null where pcId='").append(pcId).append("'");
		this.executeHql(hql.toString());
	}
